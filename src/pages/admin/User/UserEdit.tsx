...
import { Edit3, ArrowLeft, Mail, Phone, User, Lock } from "lucide-react";
import { useUserStore } from "../../../store/useUserStore";

export default function UserEdit() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>(); 
  const { users, updateUser, fetchUsers, loading } = useUserStore(); // Menambahkan fetchUsers
  ...
  // Ambil data user terbaru saat komponen dimuat (antisipasi jika reload halaman edit)
  useEffect(() => {
    fetchUsers();
  }, []);

  // Sinkronisasi data user ke dalam form state
  useEffect(() => {
    if (id && users) {
      const currentUsers = Array.isArray(users) ? users : [];
      const userToEdit = currentUsers.find((u: any) => String(u.id) === String(id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Jika password dikosongkan atau hanya berisi spasi, hapus dari payload
      if (!payload.password || payload.password.trim() === "") {
        delete payload.password;
      }

      await updateUser(Number(id), payload);
      await fetchUsers(); // ⚡️ Krusial: Ambil ulang data terupdate dari Railway agar tabel index langsung berubah!
      
      alert("Data pengguna berhasil diperbarui!");
      navigate("/admin/user"); 
    } catch (error: any) {
      console.error("Gagal memperbarui data user:", error);
      alert(error.response?.data?.message || "Terjadi kesalahan sistem saat memperbarui data.");
    }
  };
      ...
      <div className="w-full max-w-2xl mx-auto bg-white border border-slate-200/90 p-6 md:p-8 rounded-[2.2rem] shadow-[0_8px_30px_rgba(0,0,0,0.03)]">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 border-b border-slate-100 pb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-slate-900 tracking-tight md:text-2xl">Edit Informasi User</h1>
              <p className="text-xs font-medium text-slate-500 mt-0.5">Ubah kredensial, kontak, maupun hak akses akun pengguna.</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div>
            <label className="block text-xs font-extrabold uppercase tracking-wider text-slate-500 mb-1.5">
              Ganti Password Baru <span className="text-slate-400 font-normal capitalize">(Kosongkan jika tidak diubah)</span>
            </label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-11 pr-4 text-sm font-medium text-slate-800 outline-none transition focus:border-slate-500 placeholder:text-slate-400"
                placeholder="Isi jika admin ingin meriset kata sandi user..."
              />
            </div>
          </div>
          ...
          {/* ROLE SELECTOR GRID */}
          <div>
            <label className="block text-xs font-extrabold uppercase tracking-wider text-slate-500 mb-2">Pilih Tingkatan Akun / Role Baru</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div
                onClick={() => setFormData({ ...formData, role: "User" })}
                className={`border rounded-xl p-4 cursor-pointer transition flex items-start gap-3 ${
                  formData.role.toLowerCase() === "user" ? "border-blue-500 bg-blue-50/20 ring-1 ring-blue-500/20" : "border-slate-200 hover:bg-slate-50"
                }`}
              >
                <input
                  type="radio"
                  name="role"
                  checked={formData.role.toLowerCase() === "user"}
              ...
              <div
                ...
                className={`border rounded-xl p-4 cursor-pointer transition flex items-start gap-3 ${
                  formData.role.toLowerCase() === "admin" ? "border-rose-500 bg-rose-50/20 ring-1 ring-rose-500/20" : "border-slate-200 hover:bg-slate-50"
                }`}
              >
                <input
                  type="radio"
                  name="role"
                  checked={formData.role.toLowerCase() === "admin"}
