...
export default function StepKonfirmasiPembayaran({
  ...
  // ✅ FIX: guard biar onCreateBooking cuma sekali ditembak,
  // nggak bergantung ke bookingResult (yang update-nya async)
  const hasTriggeredCreate = useRef(false);

  useEffect(() => {
    if (!bookingResult && !hasTriggeredCreate.current) {
      hasTriggeredCreate.current = true;
      setCreationError("");
      onCreateBooking().catch(() => {
        hasTriggeredCreate.current = false; // izinkan retry manual kalau gagal
        setCreationError("Gagal membuat booking. Silakan kembali dan coba lagi.");
      });
    }
  }, []);
  ...
  const canProceed = isCash
    ? !creatingBooking && !creationError
    : !!bookingResult && uploaded;
  ...
  return (
    ...
    >
      ...
      {/* Error saat gagal membuat booking */}
      {creationError && (
      ...
      {/* Info loading booking di background */}
      {creatingBooking && !bookingResult && !creationError && (
        <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-2.5 flex items-center gap-2">
          ...
          <p className="text-xs text-blue-600 font-medium">
            Menyiapkan pesananmu, silakan pilih metode pembayaran sambil menunggu...
          </p>
        </div>
      )}
              ...
              return (
                ...
                >
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${
                    isSelected
                      ? "bg-gradient-to-br from-[#0050FF] to-indigo-600 text-white shadow-md shadow-blue-400/25"
                      : "bg-gray-100 text-gray-400"
                  }`}>
                    {TYPE_ICON[c.type] ?? <CreditCard size={15} />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-bold leading-tight ${isSelected ? "text-[#001845]" : "text-gray-700"}`}>
                      {c.name}
                    </p>
          ...
          <div className="px-4 pb-4 flex-1 flex flex-col gap-3">
            {!selectedChannel ? (
              ...
              <div className="flex flex-col items-center gap-3 py-4">
                ...
                <div className="w-full bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2.5 text-[11px] text-emerald-700 leading-relaxed text-center">
                  Booking akan tersimpan di riwayat transaksi profil kamu setelah proses selesai.
                </div>
              </div>
            )}
