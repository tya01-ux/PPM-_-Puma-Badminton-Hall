...
import {
  ChevronLeft, ChevronRight, CreditCard, QrCode, Building2, Wallet, Clock,
  Upload, X, CheckCircle2, ImageIcon, AlertCircle,
} from "lucide-react";
...
export default function StepKonfirmasiPembayaran({
  bookingData,
  bookingResult,
  creatingBooking,
  onCreateBooking,
  ...
  // ✅ Booking otomatis dibuat begitu step ini terbuka (kalau belum ada)
  useEffect(() => {
    if (!bookingResult) {
      setCreationError("");
      onCreateBooking().catch(() => setCreationError("Gagal membuat booking. Silakan kembali dan coba lagi."));
    }
  }, []);
  ...
  const canProceed = !!bookingResult && (isCash || uploaded);

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { setUploadError("Ukuran file maksimal 5MB."); return; }
    if (!file.type.startsWith("image/")) { setUploadError("File harus berupa gambar (JPG, PNG, dll)."); return; }
    setUploadError("");
    setProofFile(file);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: "easeOut" }} className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-bold text-base text-[#001845] leading-tight">Konfirmasi & Pembayaran</h2>
          <p className="text-xs text-gray-400 mt-0.5">Pilih metode dan selesaikan pembayaran</p>
        </div>
      </div>

      {/* Error saat gagal membuat booking (tidak menyembunyikan UI di bawahnya) */}
      {creationError && (
        <div className="bg-red-50 border border-red-100 rounded-2xl p-4 flex items-start gap-3">
          <AlertCircle size={18} className="text-red-500 flex-shrink-0 mt-0.5" />
          <div className="flex-1">
            <p className="text-sm font-semibold text-red-600 mb-1">{creationError}</p>
            <button onClick={handleRetryCreate} className="text-xs font-semibold text-red-600 underline hover:text-red-700">
              Coba lagi
            </button>
          </div>
        </div>
      ...
      {/* Info kecil kalau booking masih disiapkan di background (tidak blokir UI) */}
      {creatingBooking && !bookingResult && !creationError && (
        <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-2.5 flex items-center gap-2">
          <div className="w-3.5 h-3.5 border-2 border-[#0050FF] border-t-transparent rounded-full animate-spin flex-shrink-0" />
          <p className="text-xs text-blue-600 font-medium">Menyiapkan pesananmu, silakan pilih metode pembayaran sambil menunggu...</p>
        </div>
      )}

      {/* Countdown + Total — selalu tampil, dihitung dari bookingData */}
      <div className="bg-gradient-to-r from-[#001845] to-[#0050FF] rounded-2xl px-5 py-3.5 flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className={`text-xl font-black mt-0.5 ${timeLeft < 60 ? "text-red-300 animate-pulse" : "text-white"}`}>{formatCountdown(timeLeft)}</p>
        </div>
      </div>

      {/* Metode & Instruksi — selalu tampil, tidak menunggu bookingResult */}
              ...
              <div className="px-4 pb-4 space-y-2">
                {channels.length === 0 && (
                  <div className="flex items-center justify-center py-8 gap-2 text-gray-300">
                    <CreditCard size={20} /> <p className="text-sm">Memuat metode...</p>
                  </div>
                )}
                {channels.filter((c) => c.isActive).map((c) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${isSelected ? "bg-gradient-to-br from-[#0050FF] to-indigo-600 text-white shadow-md shadow-blue-400/25" : "bg-gray-100 text-gray-400"}`}>
              ...
              <div className="px-4 pb-4 flex-1 flex flex-col gap-3">
                {!selectedChannel ? (
                  <div className="flex-1 flex items-center justify-center text-gray-300 gap-2 py-6">
                    <CreditCard size={20} /> <p className="text-sm">Pilih metode dulu</p>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-[11px] text-gray-400 text-center leading-relaxed">Scan QR code di atas menggunakan aplikasi e-wallet atau mobile banking kamu.</p>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-3 py-4">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-gray-400 mt-1 leading-relaxed">Siapkan uang tunai sesuai total dan bayar langsung ke kasir saat tiba.</p>
                    </div>
                  </div>
                )}
                ...
                <AnimatePresence>
                  {needsProof && selectedChannel && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="border-t border-gray-50 pt-3 space-y-2">
                      ...
                      {!proofPreview ? (
                        ...
                        <div className="relative rounded-xl overflow-hidden border border-gray-200">
                          ...
                          <div className="absolute inset-0 bg-black/20 flex items-start justify-end p-2">
                            <button onClick={handleRemoveProof} className="w-7 h-7 rounded-lg bg-white/90 hover:bg-white flex items-center justify-center text-gray-700 shadow transition">
                              <X size={13} />
                            </button>
                          </div>
                          {uploaded && (
                      ...
                      <AnimatePresence>
                        {uploadError && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-xl px-3 py-2">
                      ...
                      {proofFile && !uploaded && (
                        ...
                        >
                          {uploadingProof ? (
                            <>
                              <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Mengunggah...
                            </>
                          ) : (
                            <>
                      ...
                      <AnimatePresence>
                        {uploaded && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-2 bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2.5">
                            <CheckCircle2 size={14} className="text-emerald-500 flex-shrink-0" />
                            <p className="text-xs text-emerald-600 font-medium">Bukti berhasil diunggah! Admin akan memverifikasi pembayaranmu.</p>
                          </motion.div>
                        )}
                      </AnimatePresence>
      ...
      <div className="flex gap-3 pt-1">
        ...
        >
          {isCash ? "Selesai, Lanjut" : uploaded ? "Bayar Sekarang" : "Upload Bukti Dulu"}
